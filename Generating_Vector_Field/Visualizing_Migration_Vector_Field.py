...
for i in range(len(bird_tags)): 
    ...
    for j in range(len(y_vector_field)):
        vectors = y_vector_field.iloc[j]
        for k in range(len(vectors)):
            try:
                vector = vectors[k]
                latitude = math.floor(min_lat[i]) + j
                time = k*10
                plt.quiver(time, latitude, (time+0.25), (vector*0.25 + latitude), linewidths = 0.1)
            except:
                print(k)
    plt.xlabel('time (weeks)')
    plt.ylabel('longitude')
    plt.title(str(bird_tags[i]) + " Y vector field")
    plt.savefig(str(bird_tags[i]) + "yvectorfield.png")

    x_vector_field = pd.read_excel(str(bird_tags[i] + 'yvectorfield.xlsx'))
#FOR Y VECTOR FIELDS
    # y_vector_field = pd.read_excel(str(bird_tags[i] + 'yvectorfield.xlsx'))
...
#FOR X VECTOR FIELDS (Process is exacly the same)
    ...
    # y_vector_field = pd.read_excel(str(bird_tags[i] + 'yvectorfield.xlsx'))
    # #some error is happening for certain vector fields, idk why

    # for j in range(len(x_vector_field)):
    #     vectors = x_vector_field.iloc[j]
    #     for k in range(len(vectors)):
    #         try:
    #             vector = vectors[k]
    #             longitude = math.floor(min_long[i]) + j
    #             time = k*10
    #             plt.quiver(time, longitude, (time+0.25), (vector*0.25 + longitude), linewidths = 0.1)
    #         except:
    #             print(k)
    # # for j in range(len(x_vector_field)):
    ...
    # plt.savefig(str(bird_tags[i]) + "Xvectorfield.png")
