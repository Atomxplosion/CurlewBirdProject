...
for i in range(len(bird_tags)):
    ...
    change_time = migration_values_df.iloc[5]
    ...
    for j in range(len(long_vector)-3):
        ...
        for k in range(len(t_increment)):
            ...
            start_index = x_index
            end_index = next_x_index
            if (next_x_index < x_index):
                start_index = next_x_index
                end_index = x_index
            for l in range(end_index - start_index + 1):
                #checks in case the value is infinity
                if (np.isnan(x_value_1) == False) and (np.isinf(x_value_1) == False):
                    x_vector_field[start_index+l][k].append(x_value_1)
                else:
                    x_vector_field[start_index+l][k].append(0)
                ...
                if (np.isnan(x_value_2) == False) and (np.isinf(x_value_2) == False):
                    x_vector_field[start_index+1+l][k].append(x_value_2)
                else:
                    x_vector_field[start_index+1+l][k].append(0)
    ...
    for j in range(len(x_vector_field)):
        for k in range(len(x_vector_field[0])):
            ...
            if (len(x_vector_field[j][k]) >0):
                x_vector_field[j][k] = sum(x_vector_field[j][k])/(len(x_vector_field[j][k]))
            else:
